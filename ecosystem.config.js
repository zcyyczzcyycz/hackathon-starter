const path = require('path'); // 引入 path 模块，用于拼接绝对路径
module.exports = {
  apps: [
    {
      name: 'app', // 应用唯一标识
      ...
      out_file: path.resolve(__dirname, './logs/pm2_out.log'), // 项目根目录/logs/pm2-out.log
      error_file: path.resolve(__dirname, './logs/pm2_error.log'), // 项目根目录/logs/pm2-error.log
      log_date_format: 'YYYY-MM-DD HH:mm:ss Z', // 日志时间戳格式
      env: {
        // 全局环境变量（开发默认）pm2 start 不带参数
        NODE_ENV: 'development',
        DEBUG: 'app:*',
      },
      env_production: {
        // 生产环境启动 (pm2 start --env production)
        NODE_ENV: 'production',
        PORT: 3000,
